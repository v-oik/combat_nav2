...
# ==========================================
# 설정 (차량 스펙 및 CAN Protocol)
# ==========================================
WHEEL_RADIUS_M = 0.15      # 구동륜(Sprocket) 반지름 (미터)
TRACK_WIDTH_M = 0.5        # 양쪽 궤도 중심 사이의 거리 (미터)
TRACK_SLIP_FACTOR = 0.75   # 🌟 궤도 차량 회전 슬립 계수 (실제 테스트 시 360도 회전량에 맞춰 튜닝)
LINE_TO_RAD_PER_SEC = 0.001 
...
# ==========================================
# ROS 2 노드 클래스 (/odom 발행 및 /cmd_vel 수신)
# ==========================================
class VehicleROSNode(Node):
    ...
    def cmd_vel_callback(self, msg):
        ...
        # 2. 선속도(m/s)를 모터 엔코더 값(line/s)으로 역변환
        conversion_factor = (LINE_TO_RAD_PER_SEC * WHEEL_RADIUS_M)
        left_line = left_v / conversion_factor
        right_line = right_v / conversion_factor
        
        # 3. GUI 슬라이더 계산식에 맞게 변환 (좌우 반전 반영)
        # 좌회전(v_yaw > 0)일 때 right_line이 더 크므로, steer를 양수(+)로 만들기 위해 순서 변경
    ...
    def publish_odom(self, left_line_s, right_line_s):
        ...
        left_v = left_line_s * LINE_TO_RAD_PER_SEC * WHEEL_RADIUS_M
        right_v = right_line_s * LINE_TO_RAD_PER_SEC * WHEEL_RADIUS_M
        ...
        # 🌟 평면 2D 주행(X, Y, Yaw)은 신뢰하고, 고도 및 기울기(Z, Roll, Pitch)는 무시하도록 999.0 설정
        covariance = [
...
class VehicleControl:
    def __init__(self, root):
        self.root = root
        self.root.title("TinS-13 Autonomous & Manual Control (Keyboard)")
    ...
    def init_ui(self):
        ...
        # 슬라이더 방향 반전 (왼쪽이 + 양수가 되도록 from과 to 위치 변경)
        ...
        self.status_label = ttk.Label(frame, text="Status: Ready", font=("Arial", 11, "bold"), foreground="blue")
        ...
        while not self.ui_queue.empty():
            ...
            if msg_type == "rx":
                if data.arbitration_id == FEEDBACK_CAN_ID and len(data.data) >= 4:
                    ...
                    self.status_label.config(text=f"Actual Motor -> L: {left_act} line/s, R: {right_act} line/s")
                    
                    # 🚀 수신된 바퀴 속도를 /odom 으로 발행
    ...
    def push_tx_loop(self):
        if not self.control_enabled.get():
            ...
            # 실제 CAN 명령 전송 부호 반전 (좌회전(+)일 때 좌측 감소, 우측 증가)
